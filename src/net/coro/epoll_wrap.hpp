...
public:
   ...
   * @param fd 事件描述符
   * @param ctl_opt EPOLL_CTL_*
   * @param ep_evt epoll事件结构体
   */
  void ctl(int fd, int ctl_opt, struct epoll_event &ep_evt,
           bool add_retry = false) {
    int ret;
    ...
    for (;;) {
      ret = epoll_ctl(epfd_, ctl_opt, fd, &ep_evt);
      if (!ret) {
        return;
      }
      if ((errno != EEXIST) || !add_retry) {
        /*
        ...
        时遇到/proc/sys/fs/epoll/max_user_watches施加的限制
        epoll 实例上的新文件描述符。见epoll(7)
        了解更多详情。

        ENOENT op是 EPOLL_CTL_MOD 或
        EPOLL_CTL_DEL，而fd不是注册到这个 epoll 实例。

        ELOOP fd指的是一个 epoll 实例 这个EPOLL_CTL_ADD
        操作会导致循环的epoll实例相互监视 或嵌套深度大于
        5。
        ...
        EINVAL
        1. epfd 不是 epoll的文件描述符，或者fd ==
        epollfd, 或者请求的操作 op 不被该接口支持。
        2. 一个非法的事件类型与 EPOLLEXCLUSIVE 被指定
        3. op 是 EPOLL_CTL_MOD 并且 events 包含
        EPOLLEXCLUSIVE.
        4. op 是 EPOLL_CTL_MOD 并且 EPOLLEXCLUSIVE
        之前应用到这个 <epfd, fd> 对.
        */
        /* log/throw */
        throw std::system_error{errno, std::system_category(), "epoll_ctl"};
      }
      ctl_opt = EPOLL_CTL_MOD;
    }
  }

  void try_add(int fd, struct epoll_event &ep_evt) {
    ctl(fd, EPOLL_CTL_ADD, ep_evt, true);
  }

  void add(int fd, struct epoll_event &ep_evt) {
    ctl(fd, EPOLL_CTL_ADD, ep_evt);
  }

  void mod(int fd, struct epoll_event &ep_evt) {
  ...
  int loop(struct epoll_event *__events, int __maxevents, int wait_ms) {
    int count = 0;
    do {
